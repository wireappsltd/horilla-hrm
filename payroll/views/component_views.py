...
def payslip_super_detailed_export(request):
    ...
    for payslip in payslips:
        ...
        row_values = [row.get(h, 0) for h in headers]
    ...
    for h in headers:
        if h == "Employee":
            ...
            total_row.append(round(totals[h], 2))
    ws.append(total_row)
    total_row_idx = row_count + 2  # header=1, then data rows, then total
    for cell in ws[total_row_idx]:
        cell.font = total_font

    # Empty row then generated-by info
    ws.append([])
    ...
    date_range_str = ""
    if start_date_from and start_date_till:
        date_range_str = f" | Date Range: {start_date_from} to {start_date_till}"
    elif start_date_from:
        date_range_str = f" | From: {start_date_from}"
    elif start_date_till:
        date_range_str = f" | Till: {start_date_till}"
    ws.append([f"Generated by: {generated_by} on {generated_at}{date_range_str}"])
