...
def _validate_export_date_range(request):
    """
    Server-side validation: ensure no date-range pair in the export request
    exceeds 1 month.  Returns an error message string if invalid, or None if OK.
    """
    pairs = [
        ("start_date", "end_date"),
        ("start_date_from", "start_date_till"),
        ("end_date_from", "end_date_till"),
    ]
    for field_from, field_till in pairs:
        val_from = request.GET.get(field_from, "").strip()
        val_till = request.GET.get(field_till, "").strip()
        if val_from and val_till:
            try:
                d_from = datetime.strptime(val_from, "%Y-%m-%d").date()
                d_till = datetime.strptime(val_till, "%Y-%m-%d").date()
            except ValueError:
                continue
            if d_till < d_from:
                return str(
...
def _build_export_metadata(request):
    """
    Return metadata string with generated-by name, date, time and optional
    ...
    end_date_till = request.GET.get("end_date_till", "")
    ...
    if start_date_val or end_date_val:
        date_range_parts.append(
            f"Report Period: {start_date_val or 'N/A'} to {end_date_val or 'N/A'}"
        )
    elif start_date_from or start_date_till or end_date_from or end_date_till:
        if start_date_from or start_date_till:
            date_range_parts.append(
                f"Start Date: {start_date_from or 'N/A'} to {start_date_till or 'N/A'}"
            )
        if end_date_from or end_date_till:
            date_range_parts.append(
                f"End Date: {end_date_from or 'N/A'} to {end_date_till or 'N/A'}"
            )

    date_range_text = " | ".join(date_range_parts)

    return (
        f"Generated by: {generated_by} | Date: {now.strftime('%Y-%m-%d')} "
        f"| Time: {now.strftime('%H:%M:%S')}"
        + (f" | {date_range_text}" if date_range_text else "")
    )
...
def payslip_export(request):
    ...
    date_error = _validate_export_date_range(request)
    if date_error:
        return HttpResponse(date_error, status=400)
    ...
    # Numeric column names for totals (monetary / numeric fields)
    ...
    for col_value, col_name in selected_columns:
        base_field = col_value.split("__")[0]
        if base_field in numeric_column_values:
            numeric_columns.add(col_name)

    totals = {col_name: 0 for col_name in numeric_columns}
    ...
    for payslip in payslips:
        ...
        for column_value, column_name in selected_columns:
            ...
            if type(value) == date:
                ...
                for format_name, format_string in HORILLA_DATE_FORMATS.items():
                    if format_name == date_format:
                        data = parsed_date.strftime(format_string)
            else:
                data = str(value) if value is not None else ""
            ...
            if column_name in totals:
                try:
                    totals[column_name] += float(value) if value else 0
                except (ValueError, TypeError):
                    pass

        payslips_data.append(row)
    ...
    # Write data rows
    for row_num, row_data in enumerate(payslips_data, 2):
        for col_num, header in enumerate(header_row, 1):
            cell = ws.cell(row=row_num, column=col_num, value=row_data.get(header, ""))
            cell.alignment = Alignment(horizontal="center")

    # Append totals row
    totals_row_num = len(payslips_data) + 2
    for col_num, header in enumerate(header_row, 1):
        if header in totals:
            cell = ws.cell(
                row=totals_row_num, column=col_num, value=round(totals[header], 2)
            )
        elif col_num == 1:
            cell = ws.cell(row=totals_row_num, column=col_num, value="Total")
        else:
            cell = ws.cell(row=totals_row_num, column=col_num, value="-")
    ...
    # Metadata: date, time, username, and date range at the bottom left
    metadata_row_num = totals_row_num + 2
    meta_cell = ws.cell(
        row=metadata_row_num,
        column=1,
        value=_build_export_metadata(request),
    )
    meta_cell.font = Font(italic=True, size=10, color="555555")
...
def payslip_super_detailed_export(request):
    ...
    # Collect all rows and compute totals
    numeric_headers = [h for h in headers if h != "Employee"]
    totals = {h: 0 for h in numeric_headers}
    ...
    for payslip in payslips:
        ...
        for h in headers:
            ...
            if h in totals:
                try:
                    totals[h] += float(val) if val else 0
                except (ValueError, TypeError):
                    pass
    ...
    for h in headers:
        if h == "Employee":
            totals_row_values.append("Total")
        elif h in totals:
            totals_row_values.append(round(totals[h], 2))
        else:
            totals_row_values.append("")
    ws.append(totals_row_values)

    # Bold formatting for the totals row
    ...
    # Metadata: date, time, username at the bottom
    metadata_row_idx = ws.max_row + 2
    meta_cell = ws.cell(
        row=metadata_row_idx,
        column=1,
        value=_build_export_metadata(request),
    )
    meta_cell.font = Font(italic=True, size=10, color="555555")
